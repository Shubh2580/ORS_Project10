...
public abstract class BaseDAOImpl<T extends BaseDTO> implements BaseDAOInt<T> {
	...
	protected TypedQuery<T> createCriteria(T dto, UserContext userContext) {
		...
		System.out.println(" isgroup skipped......Shubham");
		cq.where(whereClause.toArray(new Predicate[whereClause.size()]));

		List<Order> orderBys = getOrderByClause(dto, builder, qRoot);

		System.out.println("after getOrder By clause........Shubham");
		cq.orderBy(orderBys.toArray(new Order[orderBys.size()]));
		
		System.out.println("After order by clause.......Shubham ");
		TypedQuery<T> query = entityManager.createQuery(cq);
		System.out.println("createCriteria end here---....Shubham");
		return query;
	...
	public long add(T dto, UserContext userContext) {
		...
		System.out.println("before calling persist method in base dao......Shubham");
	 ...
	 */
	  ...
	  protected List<Order> getOrderByClause(T dto, CriteriaBuilder builder,
	  Root<T> qRoot) {
	 
	 // Apply Order by clause
	  System.out.println("baseDAO in getOrderByClause.......Shubham ");
	  LinkedHashMap<String, String> map = dto.orderBY();
	 
	  List<Order> orderBys = new ArrayList<Order>();
	 
	  map.forEach((key, value) -> { if (value.equals("asc")) {
